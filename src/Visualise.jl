    ...
    for i in eachindex(fadjlist)
        for j in fadjlist[i]
            if (i, j) ∉ done_idxs && (j, i) ∉ done_idxs
                ...
                if edge_in_component(i, j) edges[idx] = :([width 4 fill "#000000"])
                else edges[idx] = :([width 1 style "dotted" fill "#000000"]) end
    end end end

    Erdos.vprop!(g, "label", VertexMap(g, ["$i" for i in eachindex(fadjlist)]))

    colors::Vector{Expr} = fill(:([fill "#000000"]), length(fadjlist))
    colors_idxs::Vector{Int64} = Shuffle.shuffle(1:length(communities))
    for i in eachindex(communities)
        for j in communities[i]
            c::String = string(
                "#"*hex(RGB(cgrad(:thermal, length(communities)+1, categorical = true)[colors_idxs[i]])),
                "d9"
            )
            colors[j] = :([fill $c])
    end end
